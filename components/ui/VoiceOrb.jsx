...
/* Animated WebGL orb (adapted from the voice-powered-orb snippet).
   ONLY the visual is kept: all microphone/voice-analysis code is removed.
   The `engaged` prop (driven by touch/click in the parent) is what excites
   the orb: it spins faster and ripples while engaged, and settles back to a
   calm idle when not. RAF pauses while off-screen; time freezes under
...
export function VoiceOrb({ className, hue = 0, engaged = false }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('VoiceOrb init failed:', error)
      return undefined
    }
  }, [hue])
