...
- Media: editor toolbar uploads images (jpg/png/webp/gif ≤8MB) and videos (mp4/webm ≤200MB) DIRECTLY from the browser to Vercel Blob (client upload via @vercel/blob/client; store jotil-blog-media, public; /api/admin/upload is only the cookie-guarded token issuer — never proxy file bytes through a route, Vercel 413s large bodies) and inserts snippets at the cursor: `![alt](url)`, `<Video src="..." />`, `<YouTube id="..." />` (id parsed from any pasted YouTube link). Renderers in components/blog/MdxMedia.jsx, registered in the detail page's mdxComponents (file posts can use them too)
