...
/* Creation timestamps (epoch ms) for the admin analytics charts.
   Return null when Postgres is not configured. */
export async function intakeCreatedSince(days = 30) {
  const sql = getSql()
  if (!sql) return null
  await ensureTable(sql)
  const rows = await sql`
    SELECT created_at FROM intake_submissions
    WHERE created_at > now() - make_interval(days => ${days})
  `
  return rows.map((r) => new Date(r.created_at).getTime())
}

export async function contactCreatedSince(days = 30) {
  const sql = getSql()
  if (!sql) return null
  await ensureContactTable(sql)
  const rows = await sql`
